...
if __name__ == "__main__":
    ...
    def train_and_evaluate(num_agents,learning_rate, learning_rate_out, max_memory_length, steps_per_update, steps_per_target_update,w):
        from model.q_learning_agent import QLearningAgent
        from DQN.dqn import DQN
        import tensorflow as tf
        from collections import deque
        from DQN.operators import MellowMax, Max
        ...
        num_actions = 2
        operator = MellowMax(w, num_actions)
    ...
    filename = "grid_hyperparameters_mellowmax.pkl"
                        ...
                        for steps_per_update in script.steps_per_update:
                            for steps_per_target_update in script.steps_per_target_update:
                                ...
                                with mp.Pool(num_agents) as p:
                                    results = p.starmap(train_and_evaluate, [(num_agents,learning_rate, learning_rate_out, max_memory_length, steps_per_update, steps_per_target_update,w) for _ in range(num_agents)])
                                ...
                                if np.sum(np.array(num_episodes_to_solve) < 750) >= 1 or mean_mean_rewards > 75:
                                    values = {'episode_reward_history': results, 'omega':w}

                                    with open(path_to_save + filename, 'ab') as f:
                                        pickle.dump(values, f)
    
    path_to_save = path_to_file.replace("configs", "../results")[:-3] + "/"
    filename = "grid_best_hyperparameters_mellowmax.pkl"
