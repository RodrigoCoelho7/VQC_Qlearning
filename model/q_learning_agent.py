from vqc.data_reup_model import DataReupPQC,BaselinePQC, UniversalQuantumClassifier
import os
os.environ['CUDA_VISIBLE_DEVICES'] = ''
import tensorflow as tf
from vqc.vqc_circuits import UQC, SkolikBaseline, LockwoodBaseline

def QLearningAgent(vqc, n_layers, observables, target,state_dim, rescaling_type):
    input_tensor = tf.keras.Input(shape=(state_dim, ), dtype=tf.dtypes.float32, name='input')

    if isinstance(vqc, UQC):
        pqc = UniversalQuantumClassifier(vqc,n_layers, observables, activation='tanh')([input_tensor])
        ...
        pqc = DataReupPQC(vqc,n_layers, observables, activation='tanh')([input_tensor])
