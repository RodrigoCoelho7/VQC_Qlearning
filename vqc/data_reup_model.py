...
class BaselinePQC(tf.keras.layers.Layer):
    ...
    def __init__(self, vqc, observables, activation="linear", name="MyPQC"):
...
class DataReupPQC(tf.keras.layers.Layer):
    ...
    def __init__(self, vqc, n_layers, observables, activation="linear", name="MyPQC"):
...
class UniversalQuantumClassifier(tf.keras.layers.Layer):
    ...
    def __init__(self, vqc,n_layers,observables, activation = "linear", name = "UQC"):
        ...
        self.w = tf.Variable(
            initial_value = w_init(shape = (self.n_layers,4), dtype = "float32"),
